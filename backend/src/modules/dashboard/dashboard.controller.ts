...
export class DashboardController {
  ...
  @ApiResponse({ status: 200, description: 'Manager metrics retrieved successfully' })
  async getManagerDashboard(@CurrentUser('employeeId') employeeId: string | null) {
    if (!employeeId) {
  ...
  @ApiResponse({ status: 200, description: 'Employee metrics retrieved successfully' })
  async getEmployeeDashboard(@CurrentUser('employeeId') employeeId: string | null) {
    if (!employeeId) {
      throw new BadRequestException('No employee profile associated with this user.');
    }
