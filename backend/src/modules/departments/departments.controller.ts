import { Controller, Get, Post, Patch, Delete, Param, Body, UseGuards, HttpStatus, HttpCode } from '@nestjs/common';
...
export class DepartmentsController {
  ...
  async create(
    ...
    const data = await this.departmentsService.create(createDepartmentDto, actorUserId);
    return { data };
  }
  ...
  async update(
    ...
    const data = await this.departmentsService.update(id, updateDepartmentDto, actorUserId);
    return { data };
  }
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Department successfully deleted' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Department not found' })
  async remove(
    @Param('id') id: string,
    @CurrentUser('id') actorUserId: string,
