...
export class PayrollController {
  ...
  @ApiOperation({
    summary: 'Create a new DRAFT payroll run for month/year (HR, ADMIN)',
  })
  ...
  @ApiOperation({
    summary: 'Process a payroll run and generate employee payslips (HR, ADMIN)',
  })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Mark a processed payroll run as PAID/LOCKED (HR, ADMIN)',
  })
  @ApiResponse({ status: HttpStatus.OK, type: PayrollRunResponseDto })
  async payRun(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary:
      'Get details of a payroll run including generated payslips (HR, ADMIN)',
  })
  @ApiResponse({ status: HttpStatus.OK, type: PayrollRunResponseDto })
  ...
  @Permissions('payroll:read')
  @ApiOperation({
    summary: 'Get details of a specific payslip (self-owner, HR, ADMIN)',
  })
  @ApiResponse({ status: HttpStatus.OK, type: PayslipResponseDto })
  async getPayslip(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary: 'Get list of payroll anomalies for a run (HR, ADMIN)',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Anomalies list retrieved successfully',
  })
  ...
  @Get('payroll/runs')
  @Permissions('payroll:run')
  @ApiOperation({ summary: 'Get all payroll runs (HR, ADMIN)' })
  @ApiResponse({ status: HttpStatus.OK, type: [PayrollRunResponseDto] })
  async getAllRuns() {
    const data = await this.payrollService.getAllRuns();
    return { data };
  }
}
